...
class BreakStrategy(BaseStrategy):
    ...
    # if last_kline["Close"] < last_kline["Open"]:
        ...
        for i in range(len(self.orders_opening) - 1, -1, -1):
            ...
            if (last_kline["Close"]*1.0005 < y_up or rsi > 70  ) and order["trading_price"] + point < last_kline["Close"]:
    ...
    # else:
        ...
        for i in range(len(self.orders_opening) - 1, -1, -1):
            ...
            if (last_kline["Close"] > y_down*1.0005  or rsi < 30 ) and order["trading_price"]  > last_kline["Close"] + point:
                order["desc"]["stop_idx"] = len(chart) - 1
                order["desc"]["y"] = y_down
                order["description"] = self.description
                order.close(last_kline)
